...
const AccountsPayableModal: React.FC<AccountsPayableModalProps> = ({ isOpen, onClose }) => {
  ...
  const fetchPayableEntries = async () => {
    setLoading(true);
    try {
      // Here you would typically make an API call to fetch accounts payable data
      // For now, we'll simulate the data
      await new Promise(resolve => setTimeout(resolve, 1000));

      const mockData: PayableEntry[] = [
        {
          id: '1',
        ...
        },
        {
          ...
          paymentDate: '',
          status: 'outstanding',
          notes: 'Auto-pay scheduled for due date',
        ...
        },
        {
          ...
          paymentDate: '',
          status: 'overdue',
          notes: 'Payment reminder sent to vendor',
          createdAt: '2024-10-28'
        },
        {
          id: '4',
          vendorName: 'Cleaning Services Inc',
          expenseId: 'EXP-2024-004',
          amount: 225.00,
          dueDate: '2024-12-01',
          paymentDate: '',
          status: 'outstanding',
          notes: 'Monthly cleaning service bill',
          createdAt: '2024-11-08'
        }
      ];

      setPayableEntries(mockData);
    } catch (error) {
  ...
  const filteredEntries = payableEntries.filter(entry => {
  ...
  return (
    <div className="modal-gradient-bg rounded-lg shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col transform transition-all duration-300 ease-in-out scale-95 animate-scale-in">
      <div className="flex justify-between items-center p-4 border-b border-gray-200/80">
        <h3 className="text-2xl font-semibold text-gray-800">Accounts Payable</h3>
        <button
          onClick={onClose}
      ...
      <div className="p-6 overflow-y-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-sm text-gray-600">Manage vendor payments and outstanding bills</p>
          </div>
          <button
            onClick={() => window.dispatchEvent(new CustomEvent('openAddPayableModal'))}
          ...
          <div className="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredEntries.map((entry) => (
                    <tr key={entry.id} className={`hover:bg-gray-50 ${isOverdue(entry.dueDate, entry.status) && entry.status !== 'paid' ? 'bg-red-50' : ''}`}>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{entry.vendorName}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{entry.expenseId}</td>
            ...
            {filteredEntries.length === 0 && (
              <div className="text-center py-12">
                <p className="text-gray-500">No accounts payable entries found</p>
              </div>
            )}
          </div>
        )}
